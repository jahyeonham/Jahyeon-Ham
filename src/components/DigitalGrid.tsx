...
export const DigitalGrid: React.FC<DigitalGridProps> = ({
  ...
  return (
    <section id="digital-work-section" className="max-w-6xl mx-auto px-6 sm:px-10 py-12 sm:py-16">
      {/* Section Header */}
        ...
        <div className="flex items-baseline gap-4">
          ...
          {isAdmin && onAddNewProject && (
            ...
            >
              ...
              <span>브랜드 프로젝트 추가</span>
            </button>
          )}
        </div>
        <span className="text-[10px] uppercase tracking-[0.2em] opacity-40 font-bold text-[#1A1A1A]">
          Promotional, Commercial &amp; Online Works
        </span>
      </div>

      {/* 3-Column Grid for Promotional & Brand Works */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12 sm:gap-y-14">
        {(projects || []).filter(Boolean).map((project, index) => (
          <article
            key={project.id || `digital-${index}`}
            id={`digital-card-${project.id || index}`}
